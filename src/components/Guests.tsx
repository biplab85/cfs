...
export default function Guests() {
  ...
  // Handle "View More" click - show all guests
  const handleViewMore = () => {
    setIsLoading(true);
    setTimeout(() => {
      setVisibleCount(guestsData.length);
      setIsLoading(false);
    }, 800);
  };
  ...
  // Show YouTube button once all guests in the current filter are shown
  const showYouTubeButton = visibleCount >= allFilteredGuests.length;
